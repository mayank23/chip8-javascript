...
Chip8.prototype.opcodeC = function(opcode){
  var regIndex = (opcode & 0x0F00) >>> 8;
  var kk = (opcode & 0x00FF);
  this.V[regIndex] = (kk & (Math.random() * 255)) & 0xFF;
  this.PC+=2;
};
...
Chip8.prototype.opcodeD = function(opcode){
  ...
  for(var memoryIndex = this.I; memoryIndex < this.I + n;memoryIndex++){
    ...
    for(var shift = 7;shift >=0;shift--){
      ...
      if(this.screenState[y][x] ===1 && currentPixelStateOn === 1){
        this.V[0xF] = 1;
      }else{
        this.V[0xF] = 0;
      }
      this.screenState[y][x] ^= currentPixelStateOn;
      if(this.screenState[y][x] === 1){

        ctx.fillStyle = 'rgb(255,255,255)';
        ctx.fillRect((x*this.scale) % this.screenWidth,(y*this.scale) % this.screenHeight,this.scale,this.scale);
      }else{
    ...
    // reset x to start from left most side.
    x = ox;
    y = (y+1) % 32;
  }
  this.PC+=2;
...
Chip8.prototype.dumpMemory = function(){
    ...
    for(var i=0x200;i<0xFFF && i < 0x200 + this.currentGameProperties.currentGameByteSize;i++){
      memoryDump += this.memory[i].toString(16) + ' ';
      if(i % 20 === 0){
        memoryDump += '<br>';
      }
    }
